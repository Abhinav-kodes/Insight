...
import { Sparkles, Layers, Search, Filter, TrendingUp, BookOpen, Code2 } from 'lucide-react';
...
const HomeHeader = ({ userMode, interests }) => {
  ...
  const getHeaderInfo = () => {
    switch (userMode) {
      case 'trend_watcher':
        return { label: 'Global Trends & News', icon: TrendingUp };
      case 'hobbyist':
        return { label: 'Curated Tech Feed', icon: Code2 };
      default: // researcher
        return { label: 'Academic Research Feed', icon: BookOpen };
    }
  };

  const { label, icon: Icon } = getHeaderInfo();
        ...
        {content.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8">
            {content.map((item) => (
              <PaperCard
                key={item.id}
                {...item} // Automatically passes content_type, thumbnail, metadata
              />
            ))}
          </div>
        ) : (
          <div className="bg-white rounded-3xl p-16 text-center shadow-sm border border-[#E7D0C5] mt-8">
            ...
            <p className="text-[#5C4633] max-w-md mx-auto">
              We couldn't find content matching your tags. Try adding broader interests in your profile.
            </p>
          </div>
        )}
      </div>
